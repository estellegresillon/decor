...
const thumbsImages = [];
contentElems.forEach((item) => {
  const thumbs = [...item.querySelectorAll(".content__thumbs-item")];
  thumbsImages.push(thumbs);
});
...
const displayImage = (item, thumb) => {
  const img = thumb.style.backgroundImage;
  item.DOM.imageInner.style.backgroundImage = img;
};
...
const initEvents = () => {
  for (const [pos, item] of previewItems.entries()) {
    item.DOM.imageWrap.addEventListener("click", () => {
      ...
      if (currentItem !== -1) {
        for (const [_, thumb] of item.content.DOM.thumbs.entries()) {
          thumb.addEventListener("click", () => {
            if (isAnimating) return;
            displayImage(item, thumb);
          });
        }
      }
  ...
  backCtrl.addEventListener("click", () => {
    ...
    // reinitialize image on close
    if (currentItem !== -1) {
      displayImage(previewItems[currentItem], thumbsImages[currentItem][0]);
    }
  });
};
